...
pub async fn get_state_handler(
    ...
    let response = service::translate_get_state(client.as_ref(), &book_id, &token.api_key)?;
...
pub async fn update_state_handler(
    ...
    let response = service::translate_update_state(client.as_ref(), &book_id, state, &token.api_key)?;
